...
export async function debugInfo({ ctx, log, includeProject }: GetDebugInfoParams): Promise<DebugInfo> {
  ...
  const entry = log.info({
    section: ctx.provider.name,
    msg: "collecting provider configuration",
    status: "active",
  })
  ...
  const namespaces = await Bluebird.map(namespacesList, async (ns) => {
    const nsEntry = entry.info({
      section: ns,
      msg: "collecting namespace configuration",
      status: "active",
    })
    const out = await kubectl.stdout({
      log,
      provider,
  ...
  const version = await kubectl.stdout({
    log,
    provider,
